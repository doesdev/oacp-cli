...
module.exports = (resourceList, options) => {
  resources = resourceList
  console.log('pgfuncOpts', options)
  schema = (typeof options.name === 'function') ? schema : options.name
...
function PgFunc (resource) {
  console.log(`Started on pgfuncs for ${resource}`)
  var self = this
  self.columns = {core: []}
...
PgFunc.prototype.getColumns = function () {
  ...
  var query = '' +
    'SELECT column_name, column_default, udt_name::regtype AS data_type ' +
    'FROM information_schema.columns ' +
    'WHERE table_schema = $1 AND table_name = $2;'
  console.log(query, schema, resource)
  self.pg
...
PgFunc.prototype.writePgFunctions = function () {
  var self = this
  if (self.columns.core.length === 0) {
    return console.error(`No columns found for ${self.resource}`)
  }
  var resource = self.resource
  var columns = self.columns
  ...
  ['search', 'create', 'read', 'update', 'delete'].forEach((action) => {
    if (!bpFuncs[action]) {
      return console.error(`No matching asset found for ${action}`)
    }
    columnList =
